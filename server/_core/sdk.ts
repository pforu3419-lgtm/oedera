...
export type SessionPayload = {
  ...
  appId: string;
  name: string;
  role?: string; // เพิ่ม role ใน JWT token
};
...
class SDKServer {
   ...
   * @example
   * const sessionToken = await sdk.createSessionToken(userInfo.openId, { name: "John", role: "admin" });
   */
    ...
    return this.signSession(
      {
        ...
        appId: ENV.appId,
        name: options.name || "",
        role: options.role, // ฝัง role ใน token
      },
      options
    );
  }
  ...
  async signSession(
    ...
    return new SignJWT({
      ...
      appId: payload.appId,
      name: payload.name,
      role: payload.role, // ฝัง role ใน JWT
    })
  ...
  async verifySession(
    ...
    try {
      ...
      return {
        ...
        appId,
        name,
        role: typeof role === "string" ? role : undefined, // return role ถ้ามี
      };
    } catch (error) {
